...
def lstm_run(X_train, X_test, y_train, y_test, epochs):
    ...
    #
    #
    # torch.save(model, 'lstm_model.pth') # save the entire model


    model = torch.load(r'E:\fmri\python分类\test2\result\LSTM-test\0.7316.pth')
    # 开始验证
    model.eval()
...
def train(epoch, sample):
    ...
    file = 'result/LSTM-test/savemodel.csv'  #Epoch=50, 五折交叉验证
